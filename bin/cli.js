...
config.renamePath = name => {
  if (name.indexOf('/target/') > 0) return name.replace('/target/', '/');
  return name;
};

config.getNodeAttributes = (file) => {
  let group;
  if (file.startsWith('src/plugins/')) group = file.replace(/^(src\/plugins\/[^\/]+)\/.*$/, '$1');
  else if (file.startsWith('src/')) group = file.replace(/^(src\/[^\/]+)\/.*$/, '$1');
