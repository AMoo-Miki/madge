...
function createGraph(modules, circular, config, options) {
	...
	Object.keys(modules).forEach((id) => {
		if (!nodes[id]) {
			if (!groups[id]) groups[id] = config.getNodeAttributes?.(id);

			if (groups[id]) {
				if (!subgraphs[groups[id]]) subgraphs[groups[id]] = g.subgraph(groups[id], { cluster: true });
				nodes[id] = subgraphs[groups[id]].createNode(id);
			} else {
		...
		modules[id].forEach((depId) => {
			if (!nodes[depId]) {
				if (!groups[depId]) groups[depId] = config.getNodeAttributes?.(depId);

				if (groups[depId]) {
					if (!subgraphs[groups[depId]]) subgraphs[groups[depId]] = g.subgraph(groups[depId], { cluster: true });
					nodes[depId] = subgraphs[groups[depId]].createNode(depId);
				} else {
